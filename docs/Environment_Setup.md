...
### Step 3: Enable CAN Bus Interface
You need to enable the MCP2515 CAN controller used by the Waveshare HAT. Open the config file:
```bash
...
Update your system and install the required libraries, including `can-utils` for testing.
```bash
sudo apt update && sudo apt upgrade
sudo apt install python3-venv git can-utils
```
...
1.  Download the ARM toolchain for your operating system.
2.  Synchronize the Raspberry Pi's `sysroot` (system libraries and headers) to your Host PC.
3.  Configure a new "Kit" and "Device" in Qt Creator that uses the cross-compiler and the synchronized sysroot.
