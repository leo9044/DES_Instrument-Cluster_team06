...
Rectangle {
    ...
    // CanInterface에서 속도 수신 (cm/s 사용)
    Connections {
        target: canInterface
        // 시그널: speedDataReceived(speedCms, speedKmh)
        onSpeedDataReceived: function(speedCmsArg, speedKmhArg) {
            // 계기판 그래픽 눈금(0~240) 유지하되, 값은 cm/s를 그대로 사용
            speed = Math.min(Math.round(speedCmsArg), 240)
        }
    }
    ...
    Image {
        ...
        transform: Rotation {
            origin.x: 130
            origin.y: 33
            // 눈금 0~240 → 각도 -45° ~ +225° (총 270°)
            // 여기서 speed는 cm/s 값(최대 240으로 캡)
